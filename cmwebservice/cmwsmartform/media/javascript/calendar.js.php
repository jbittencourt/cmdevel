 ...
 * Last Modified..: $Date: 2006/11/29 18:41:23 $
...
function dynCalendar_show()
{
  ...
  monthnames = new Array(<? echo "'$_language[january]','$_language[february]','$_language[march]','$_language[april]','$_language[may]','$_language[june]','$_language[july]','$_language[august]','$_language[september]','$_language[october]','$_language[november]','$_language[december]'"; ?>);
  ...
  prevImgHTML  = '<img src="<? echo "$_CMAPP[images_url]/".(string) $_conf->cmdevel->smartform->calendar->images['left']; ?>" alt="<<" border="0" />';
  prevLinkHTML = '<a href="javascript: ' + this.objName + '.show(' + previousMonth + ', ' + previousYear + ')">' + prevImgHTML + '</a>';
  nextImgHTML  = '<img src="<? echo "$_CMAPP[images_url]/".(string) $_conf->cmdevel->smartform->calendar->images['right'];?>" alt=">>" border="0" />';
  ...
  html += '<tr>';
  html += '<td class="dynCalendar_dayname"><? echo "$_language[sun]";?></td>';
  html += '<td class="dynCalendar_dayname"><? echo "$_language[mon]";?></td>';
  html += '<td class="dynCalendar_dayname"><? echo "$_language[tue]";?></td>';
  html += '<td class="dynCalendar_dayname"><? echo "$_language[wed]";?></td>';
  html += '<td class="dynCalendar_dayname"><? echo "$_language[thu]";?></td>';
  html += '<td class="dynCalendar_dayname"><? echo "$_language[fri]";?></td>';
  html += '<td class="dynCalendar_dayname"><? echo "$_language[sat]";?></td></tr>';
  html += '<tr>' + ret.join('</tr>\n<tr>') + '</tr>';
  html += '</table>';
...
{
  if (is_ie5 || is_ns5 || is_dom) {
    document.write('<a href="javascript: ' + this.objName + '.show()"><img src="<?echo "$_CMAPP[images_url]/".(string) $_conf->cmdevel->smartform->calendar->images['main']; ?>" border="0" width="16" height="16" /></a>');
...
{
  this._getLayer().style.top  = (dynCalendar_mouseY + this.offsetX) + 'px';
  this._getLayer().style.left = (dynCalendar_mouseX + this.offsetY) + 'px';
